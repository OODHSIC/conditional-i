...
class HSICB(HSIC):
    ...
    def biased_estimator(self, input1, input2):
        ...
        Gretton, Arthur, et al. "Measuring statistical dependence with Hilbert-Schmidt norms." 2005.
        """
        K = self._kernel_x(input1)  # (K, N, N)
        L = self._kernel_y(input2)  # (K, N, N)

        KH = K - K.mean(1, keepdim=True)  # (K, N, N)
        LH = L - L.mean(1, keepdim=True)  # (K, N, N)

        N = input1.size(1)

        KHLH = torch.bmm(KH, LH)  # (K, N, N)

        return KHLH.diagonal(dim1=-2, dim2=-1).sum(-1) / (N - 1) ** 2


class RbfHSICB(HSICB):
    """Radial Basis Function (RBF) kernel HSIC implementation.
    """
    def _kernel(self, X, sigma=None):
        XX = torch.bmm(X, X.permute(0, 2, 1).contiguous())  # (K, N, N)
        X_sqnorms = XX.diagonal(dim1=-2, dim2=-1)  # (K, N)
        X_L2 = -2 * XX + X_sqnorms[:, :, None] + X_sqnorms[:, None, :]  # (K, N, N)
        gamma = 1 / (2 * sigma ** 2)
