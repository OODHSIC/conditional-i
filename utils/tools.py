...
class SingleLoopSampler(Sampler):

    def __init__(self, dataset, inloop_len, shuffle=True, seed=0):
        super(SingleLoopSampler, self).__init__(dataset)
        assert inloop_len is not None
        self.dataset = dataset

        self.epoch = 0
        self.num_samples = len(self.dataset)
        self.shuffle = shuffle
        self.inloop_len = inloop_len
    ...
    def __iter__(self):
        start_idx = self.epoch * self.inloop_len
        end_idx = (self.epoch + 1) * self.inloop_len
...
def gen_cls_inds(total_it, args):
    offset = (args.sample_cls * total_it) % args.bank_cls_num
    if args.shuffle_cls:
        seed = (args.sample_cls * total_it) // args.bank_cls_num
        if offset > (args.bank_cls_num - args.sample_cls):
            seed += 1
        g = torch.Generator()
        g.manual_seed(seed)
        cls_inds = torch.randperm(args.bank_cls_num, generator=g).float()
    else:
        cls_inds = torch.arange(args.bank_cls_num).float()
    cls_inds += offset
    invalid_inds = (cls_inds >= args.bank_cls_num)
    cls_inds[invalid_inds] -= args.bank_cls_num
    return cls_inds.long()[:args.sample_cls]